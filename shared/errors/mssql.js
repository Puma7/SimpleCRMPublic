...
exports.getFriendlyMssqlError = getFriendlyMssqlError;
const MSSQL_ERROR_MAP = {
    ETIMEOUT: {
 ...
 */
function getFriendlyMssqlError(error, lang = 'de') {
    let errorCode;
    let errorName;
    let errorMessage = lang === 'de' ? 'Ein unbekannter Fehler ist aufgetreten.' : 'An unknown error occurred.';
    ...
    let matchedDetailKey = errorCode;
    if (errorCode && MSSQL_ERROR_MAP[errorCode]) {
    ...
    if (matchedDetailKey && MSSQL_ERROR_MAP[matchedDetailKey]) {
        ...
        return {
            ...
            code: errorCode,
            name: errorName,
            actionableAdvice: lang === 'de' ? detail.actionable_advice_de : detail.actionable_advice_en,
