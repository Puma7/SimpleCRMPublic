...
- Mailbox regex search (`search=/…/`) runs with a 10 s `statement_timeout`, and each user may have at most two running at once (429 otherwise). That counter is process-local: with several API processes the limit applies per process.
- The runtime database role owns the schema. `simplecrm_app` serves both the API and the migrations, so it owns the database, the `public` and `app` schemas, every table and policy, and the RLS helper functions such as `app.can_access_workspace()`. It is `NOSUPERUSER` without `BYPASSRLS`, and `FORCE ROW LEVEL SECURITY` makes the policies apply to it as owner — but that is a self-restriction: an owner may run `ALTER TABLE … NO FORCE ROW LEVEL SECURITY`, `DROP POLICY`, `CREATE OR REPLACE FUNCTION app.can_access_workspace(…)`, `DROP` or `TRUNCATE`. RLS therefore guards against a forgotten `WHERE` clause, not against SQL injection or a compromised API process; the `app.*` settings that open cross-workspace access are plain session settings any role can set, so this holds regardless of ownership. What ownership adds is persistence (a weakened policy or helper survives restarts and code fixes) and destructive DDL. No concrete injection path is known; this is defense in depth, accepted for now. Planned remedy: a separate owner role (for example `simplecrm_owner`, used only by migrations) owns all objects, and `simplecrm_app` becomes a pure runtime role with DML rights granted through `ALTER DEFAULT PRIVILEGES`. Existing installations move ownership once, as the admin role in a maintenance window, with `REASSIGN OWNED BY simplecrm_app TO simplecrm_owner`; `PG_RESTORE_ROLE` and the restore drill then switch to the owner role, and doctor/startup should check that the runtime role owns nothing.
