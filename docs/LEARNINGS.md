...
24. **Updates** — server: `sh docker/simplecrm update --version latest` (release tag, backup, health wait, two image generations `gen-*`, build cache budget, `simplecrm rollback` without rebuild, protected pre-update backup, `simplecrm disk`); `VERSION` stays the compose image tag, the release goes in `RELEASE`. Docker build cache was the real disk hog (17.5 GB on a 38 GB server); desktop: the packaged app snapshots `database.sqlite` to `backups/pre-update/` before the first start of a new version (`electron/maintenance/pre-update-backup.ts`). User guide: README „Aktualisieren“.
