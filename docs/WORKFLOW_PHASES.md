...
- Entwurfs-Herkunft setzen die Knoten, die Entwürfe anlegen: `ai.draft_reply`, `ai.agent`, `ai.pick_canned` ⇒ `ai`; `email.create_draft`, Weiterleitungskopie ⇒ `workflow`; `send_draft`/`release_outbound` (ohne menschlichen Akteur) nur, wenn noch keine Herkunft steht. Eine inhaltliche Änderung über Compose/`updateComposeDraft` setzt `draft_origin_edited`.
