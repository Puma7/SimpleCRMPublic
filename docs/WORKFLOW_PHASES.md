...
| Server: Zeitplan-Auslöser — Minutentakt `workflow.schedule.tick` je Workspace (Maintenance-Ticker), Anspruch je Zeitpunkt per bedingtem UPDATE auf `email_workflows.schedule_last_slot_at` (Migration `0056`), `workflow.execute` mit Ausloeser `schedule`, Dienst-Provenienz und Job-Key Workflow+Zeitpunkt; der Lauf beansprucht seinen Zeitpunkt zusätzlich selbst (`scheduleSlot` im Job, sync_info `workflow_schedule_run:<id>` in der Lauf-Transaktion), damit auch eine gespeicherte, aber nicht bestätigte Einreihung nur einmal läuft; Nachholung höchstens 15 Minuten; Zeitzone `workflow_schedule_timezone` (sync_info, Standard `Europe/Berlin`); Cron-Parser rein in `packages/core/src/workflow/cron-schedule.ts` (5 Felder, Mindestabstand 15 Minuten, Tag UND Wochentag wie node-cron, Paritätstest gegen node-cron); `schedule_last_slot_at IS NULL` = nicht scharf (Bestand, Desktop-Import) — scharf erst durch Anlegen/Aktivieren/Speichern im Server, Editor-Hinweis über `scheduleLastSlotAt`; „Jetzt ausführen“ läuft als `schedule` | ✅ |
