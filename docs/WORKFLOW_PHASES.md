...
- Entwurfs-Herkunft setzen die Knoten, die Entwürfe anlegen: `ai.draft_reply`, `ai.agent`, `ai.pick_canned` ⇒ `ai`; `email.create_draft`, Weiterleitungskopie ⇒ `workflow`; `send_draft`/`release_outbound` (ohne menschlichen Akteur) nur, wenn noch keine Herkunft steht. Speichern über das Entwurfsfenster (`UpdateComposeDraft` bzw. `PATCH …/compose-draft`) setzt `draft_origin_edited` nur bei einem echten Unterschied (`draftContentChanged`: Betreff, Text ohne Leerraum-/HTML-Unterschiede und ohne Hinweis „Versand blockiert“ — bei HTML mit Zonen-Markern des Entwurfsfensters nur Anrede und Text, Signatur- und Zitat-Zone zählen nicht —, Empfänger-Adressen, Anhänge, Konto) — das Fenster speichert vor jedem Senden alle Felder.
