...
- **Schedule workflows** (trigger „Zeitplan (Cron)“) need the job worker as well: every API process enqueues one `workflow.schedule.tick` job per workspace and minute (collapsed per workspace by its Graphile job key), the worker claims each due slot once in `email_workflows.schedule_last_slot_at` and queues the run; the run itself claims its slot again (`sync_info` key `workflow_schedule_run:<id>`, same transaction as the run), so a slot whose enqueue was stored but not acknowledged still runs only once. The container itself stays on UTC; cron expressions are evaluated in the workspace time zone set under **Einstellungen → Automatisierung** (default `Europe/Berlin`). Slots missed while the stack was down are only caught up if they are at most 15 minutes old.
- **Update note (schedule workflows):** after updating to a version with server-side schedules (migration `0056_email_workflow_schedule_state`), existing schedule workflows — including active ones and workflows imported from the desktop — are **not armed** and never fire on their own. Nothing starts running unexpectedly after the update. An admin arms each one by saving (or activating) it once in the workflow editor, which shows „Zeitplan ist noch nicht scharf geschaltet“ for such workflows. A repeated desktop import disarms the imported schedules again.
