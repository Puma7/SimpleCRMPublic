...
- **Schedule workflows** (trigger „Zeitplan (Cron)“) need the job worker as well: every API process enqueues one `workflow.schedule.tick` job per workspace and minute (collapsed per workspace by its Graphile job key), the worker claims each due slot once in `email_workflows.schedule_last_slot_at` and queues the run. The container itself stays on UTC; cron expressions are evaluated in the workspace time zone set under **Einstellungen → Automatisierung** (default `Europe/Berlin`). Slots missed while the stack was down are only caught up if they are at most 15 minutes old.
