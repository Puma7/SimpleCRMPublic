...
- The plan file exists and its dependencies show DONE in the plan directory's index (`plans/README.md`, or `advisor-plans/README.md` if the backlog was placed there because `plans/` was already taken). If not: stop, name the missing dependency.
...
> `README.md` — your reviewer maintains the index. Before reporting, audit every claim in
