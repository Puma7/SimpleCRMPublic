...
- **APPROVED** (approved in an unmerged worktree) — read the commit SHA recorded in the index row when it was approved, and check whether it has since merged: if it's now reachable from HEAD (`git merge-base --is-ancestor <recorded-commit> HEAD`), promote the row to DONE; if not, leave it APPROVED and remind the user it's still unmerged (dependents can't build on it yet). If the recorded commit is gone (worktree branch discarded without merging), mark it back to TODO. If no SHA was recorded — an older APPROVED row from before this was tracked — flag it to the user rather than guessing.
