...
Spawn **one** `general-purpose` subagent with `isolation: "worktree"`. **Base the worktree on the plan's stamped commit, not the default branch:** an isolated worktree defaults to `worktree.baseRef: fresh`, which branches from `origin/<default-branch>` and excludes unpushed local HEAD — so when the plan was written on a feature branch or an unpushed HEAD, pass `worktree.baseRef: head` (or otherwise check out the plan's `Planned at` SHA inside the worktree) so the executor edits the code the plan was actually stamped against. Otherwise it starts from the wrong base and hits false drift failures.

**Exception — plans with merged dependencies:** if this plan `Depends on` others that are now DONE (merged), the shared `Planned at` SHA *predates* their code, so basing the worktree on it hands the executor a tree that's missing the dependency — implementation and verification then run against the wrong state even though the precondition said the dependency is "present on the branch." In that case base the worktree on the commit that actually contains the dependencies (the current tip of the branch they merged into, e.g. `origin/<default-branch>`), and reconcile this plan first — refresh its `Planned at` SHA and Current-state excerpts to that base — so the drift check and the executor both operate on the tree that includes the merged dependencies.

Executor model: default `sonnet`; use what the user named if they named one (`execute 003 haiku`).
...
| Verdict | When | Action |
|---|---|---|
| **APPROVE** | Criteria pass, scope clean, quality holds | Set index status to **APPROVED** — *not* DONE: the work is approved but still lives in an unmerged worktree, so its code isn't on the base branch yet and a dependent plan must not treat it as available. **Record the executor's HEAD SHA and its branch/worktree in the index status cell** (e.g. `APPROVED — commit abc1234, branch advisor/002-slug`); a later `reconcile` in a fresh session has no other durable handle to test whether the work has since merged. Present to the user: diff summary, worktree path and branch, anything from NOTES. **Merging is the user's decision — never merge, push, or commit to their branch.** The row is promoted to **DONE** only once the branch is merged (a later `reconcile` pass confirms the recorded commit is on HEAD). |
...
- **APPROVED** (approved in an unmerged worktree) — read the commit SHA recorded in the index row when it was approved and decide whether the work has landed. Cheap check first: if the recorded commit is reachable from HEAD (`git merge-base --is-ancestor <recorded-commit> HEAD`), promote the row to DONE. If it's **not** reachable, do not conclude "unmerged" yet — a squash or rebase merge rewrites the commit, so the same changes can already be on HEAD under a different SHA, and an ancestry test alone would strand the plan at APPROVED forever (or wrongly reset it to TODO) and keep dependents blocked. Fall back to a content check: re-run this plan's done criteria against HEAD (do the described changes exist — the `grep` gates pass, the new tests present and green?). If they hold, the work merged (squashed/rebased) → promote to DONE. Only when the commit is unreachable **and** the done criteria don't hold is it genuinely unmerged: leave it APPROVED if the recorded branch is still open, or mark it back to TODO if that branch is gone. If no SHA was recorded — an older APPROVED row from before this was tracked — flag it to the user rather than guessing.
