/**
 * POP3 UIDL tracking: avoid unbounded persisted UIDL growth.
 * Skip-Set comes from stored messages; persisted JSON is only the current server UIDL list.
 */

/** Upper bound for serialized UIDL JSON on the folder row. */
export const POP3_UIDL_PERSIST_MAX = 20_000;

/** Serialize UIDLs reported by the server for this sync, sorted, deduped, and capped. */
