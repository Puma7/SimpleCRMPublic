...
export function normalizeMailboxName(value: string | null | undefined): string {
  return (value ?? '')
    .normalize('NFKD')
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/[_-]+/g, ' ')
...
function pathLeaf(pathValue: string, delimiter: string | undefined): string {
  ...
  for (const delimiterValue of delimiters) {
    const idx = leaf.lastIndexOf(delimiterValue);
    if (idx >= 0) {
      leaf = leaf.slice(idx + delimiterValue.length);
    }
  }
  return leaf;
}
...
export function resolveSentMailboxCandidates(
  ...
  for (const folder of ['Sent', 'Sent Items', 'Sent Mail', 'Gesendet', 'Gesendete Objekte']) {
    ...
    for (const delimiterValue of delimiters) {
      pushUnique(candidates, seen, `INBOX${delimiterValue}${folder}`);
    }
  }
...
/** First candidate that exists on the server, using case-insensitive path match. */
export function pickFirstMailboxPathOnServer(
  ...
  const paths = new Set(listedMailboxes.map((entry) => entry.path.toLowerCase()));
