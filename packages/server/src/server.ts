...
import { startAttachmentDedupTicker } from './mail-attachment-dedup';
import { startRawPartGcTicker } from './mail-raw-part-gc';
...
export async function startServer(options: ServerListenOptions = {}): Promise<FastifyInstance> {
  ...
  try {
    ...
    if (db) {
      ...
      maintenanceTickers.push(startAttachmentDedupTicker({ db, attachmentsRoot }));
      maintenanceTickers.push(startRawPartGcTicker({ db, attachmentsRoot }));
