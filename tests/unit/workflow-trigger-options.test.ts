...
describe('workflow trigger options in the editor', () => {
  test('server edition offers only triggers the server fires', () => {
    ...
    expect(values).toEqual(['inbound', 'outbound', 'manual', 'relay', 'webhook.incoming']);
  ...
  test('server edition keeps a stored desktop-only trigger visible and marks it', () => {
    ...
    expect(options).toContainEqual({ value: 'schedule', label: 'Zeitplan (Cron) (nur Desktop)', desktopOnly: true });
    expect(options.map((option) => option.value)).not.toContain('task.due');
  });
