...
describe('email-workflow-graph-compile', () => {
  ...
  // C-A82: Wieder zusammenlaufende Bedingungen wurden ungebremst in 2^n Regeln aufgefaechert (20 Ebenen: rund 1 Mio. Regeln, Sekunden im Main-/Renderer-Prozess).
  it('rejects a path explosion with a clear error instead of materialising 2^n rules', () => {
    const started = Date.now();
    expect(() => compileGraphToDefinition(reconvergingChain(20))).toThrow('Workflow-Graph zu komplex');
    expect(Date.now() - started).toBeLessThan(1_000);
  });

  it('still compiles a branched graph with 1024 paths completely', () => {
